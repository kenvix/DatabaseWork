...
object ServerEnv : ManagedEnvFile() {
    ...
    val DatabaseName by envOf("bookmgr")
    @Description("MySQL 数据库用户")
    val DatabaseUser by envOf("root")
    @Description("MySQL 数据库密码")
    val DatabasePassword by envOf( "")

    @Description("是否启用 MongoDB 数据库")
    val MongoEnabled: Boolean by envOf(true)
    @Description("MongoDB 数据库地址")
    val MongoHost: String by envOf("127.0.0.1")
    @Description("MongoDB 数据库端口")
    val MongoPort: Int by envOf(27017)
    val MongoDatabaseName by envOf("bookmgr")
    @Description("MongoDB 数据库名称")
    val MongoAuthEnabled by envOf(false)
    @Description("MongoDB 数据库用户")
    val MongoUser by envOf("")
    @Description("MongoDB 数据库密码")
    val MongoPassword by envOf("")
    @Description("MongoDB 数据库验证源")
    val MongoAuthSource by envOf("user")

    val NodeUserMaxCacheSeconds by envOf(1000L)
